  ...
  protected $dataChanged = false;
    ...
    public function initialize($options = [])
    {
        ...
        if (isset($this->options['cache']) && $this->options['cache']['class']) {
            $params = is_array($this->options['cache']['param']) ? $this->options['cache']['param'] : [];
            $this->cache = new $this->options['cache']['class']($params);
        } else {
        ...
        if (empty($this->id)) {
            ...
            // load data from cache. Watch out for the default case.
